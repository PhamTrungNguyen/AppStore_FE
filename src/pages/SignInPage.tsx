...
const SignInPage = () => {
  ...
  const handlerSubmitFrom = async (values: any) => {
    try {
      ...
      const result: any = await handlerAPIPost(
        "http://localhost:8080/api/auth/loginAccount",
        data
      );
  ...
  const responseGoogle = async (response: any) => {
    try {
      ...
      const result: any = await handlerAPIPost(
        "http://localhost:8080/api/auth/loginAccountGoogle",
        data
      );
      if (result) {
        sessionStorage.setItem("token", result.data?.token);
        ...
        <div className="text-center  inline-block ">
          <form autoComplete="off" onSubmit={handleSubmit(handlerSubmitFrom)}>
            ...
            <div className="my-[10px]">
              <GoogleLogin
                clientId="879686866490-h6lrohu4tol65rg2m4d23j92d17gd1an.apps.googleusercontent.com"
